DELIMITER $$
CREATE DEFINER=`omart`@`%` PROCEDURE `geodistance_poi_notification_v2`(IN orig_latitude double, IN orig_longitude double, IN radius int , IN paging int, IN size int, IN categoryIds VARCHAR(3000),IN provinceId int,IN districtIds VARCHAR(3000))
BEGIN
...
WHERE (categoryIds = 'ALL' OR FIND_IN_SET(pn.cat_id,categoryIds)) 
        ...
        AND pn.notification_type IN (0, 1) # 2018/06/06: Only receive SYS & SHOP NOTI
