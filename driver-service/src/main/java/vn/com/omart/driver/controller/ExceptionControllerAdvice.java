...
import vn.com.omart.driver.dto.response.ErrorResponse;
...
public class ExceptionControllerAdvice {
	...
	public ResponseEntity<ErrorResponse> exceptionHandler(Exception ex) {
		...
		return new ResponseEntity<ErrorResponse>(
				new ErrorResponse(DriverResponse.UNEXPECTED.getCode(),
						messageSourceAccessor.getMessage(DriverResponse.UNEXPECTED.getMessageCode())),
	...
	public ResponseEntity<ErrorResponse> processValidationError(MethodArgumentNotValidException ex) {
		...
		ErrorResponse errorResponse = new ErrorResponse(DriverResponse.ENTITY_INVALID.getCode(),
				description.toString());
		return new ResponseEntity<ErrorResponse>(errorResponse, HttpStatus.PARTIAL_CONTENT);
	}

	/**
	 * Handle messenger exception {@link DriverException}.
	 * 
	 * @param ex
	...
	public ResponseEntity<ErrorResponse> DriverException(DriverException ex) {
		DriverResponse DriverResponse = ex.getDriverResponse();
		return new ResponseEntity<ErrorResponse>(
				new ErrorResponse(DriverResponse.getCode(), messageSourceAccessor
						.getMessage(DriverResponse.getMessageCode(), ex.getParameters().toArray())),
				DriverResponse.getHttpStatus());
	}
}